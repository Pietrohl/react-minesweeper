...
const Board: React.FC = () => {
  ...
  function handleCellClick(row: number, column: number): void {
    const { boardMap } = boardState
    let newBoardMap = boardMap

    if (playingState === 0) onStart()
  ...
  useEffect(() => {
    ...
    if (hiddenArray === mineArray) onWin()

    if (flagsArray === mineArray) onWin()
  }, [boardState])

  useLayoutEffect(() => {
    if (playingState === PlayingState.Iddle) setBoardState(initBoardState)
  }, [playingState, level])
  ...
  const incrementNeighbors = (row: number, column: number) => {
    for (let i = Math.max(0, row - 1); i <= Math.min(rows - 1, row + 1); i++) {
      for (
        ...
        if (row !== i || column !== j) board[i][j].neighbour++
      }
    }
  }
...
const getHidden = (board: CellDataProps[][]) => {
  const hiddenArray: CellDataProps[] = []
...
const getFlags = (board: CellDataProps[][]): [number, string] => {
  ...
  const flagsArray = JSON.stringify(arr)
